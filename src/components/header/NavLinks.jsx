        ...
        if (isMobile) {
          return (
            <motion.button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg text-left transition-colors group focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 ${
                isActive
            ...
            >
              ...
              <span className={`font-medium ${isActive ? 'text-green-700 dark:text-green-300' : 'text-gray-700 dark:text-gray-300'}`}>{item.label}</span>
              <span className={`ml-auto transition-opacity ${isActive ? 'opacity-100 text-green-500' : 'opacity-0 group-hover:opacity-100 text-gray-400'}`}>{isActive ? '●' : '→'}</span>
            </motion.button>
          )
        } else {
          // Desktop nav link
